...
class ClinicalShiftApplicator:
    ...
    def __init__(self, shift_type: str, admin_data: pd.DataFrame):
    ...
    def apply_shift(self, X):
        ...
        X = X[np.in1d(X.index.get_level_values(0),self.admin_data['encounter_id'])]
    
        X_s, X_t = self.shift_types[self.shift_type](
                X, self.admin_data, **get_args(self.shift_types[self.shift_type], kwargs)
            )
