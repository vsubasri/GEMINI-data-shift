...
class RollingWindow:
    ...
    def mean(self, X: dict, window: int = 30):
        """Get rolling mean of time series data.
        ...
        """
        return pd.DataFrame.from_dict(X).rolling(window).mean().dropna().to_dict()

    def stdev(self, X: dict, window: int = 30):
        """Get rolling standard deviation of time series data.
        ...
        """
        return pd.DataFrame.from_dict(X).rolling(window).stdev().dropna().to_dict()
    ...
    def performance(
        ...
        while i + stat_window + lookup_window < len(data_streams["X"]):
            ...
            pred_metrics['sample_size'] = len(y_test_labels)
            pred_metrics['n_outcome'] = len(y_test_labels[y_test_labels == 1])
            pred_metrics['outcome_prop'] = len(y_test_labels[y_test_labels == 1])/len(y_test_labels[y_test_labels == 0])
    ...
    def drift(
        ...
        **kwargs
        ...
        while i + stat_window + lookup_window < len(data_streams["X"]):
            ...
            if randomize == 1:
                np.random.shuffle(X_next)
            
            drift_metrics = self.shift_detector.detect_shift(X_next, sample, **kwargs)

            if drift_metrics["p_val"] < threshold and verbose == 1:
                print(
