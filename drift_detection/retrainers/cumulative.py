...
from drift_detection.gemini.utils import process, scale
import numpy as np
...
class CumulativeRetrainer:
    ...
    def retrain(
            ...
            if p_val < threshold:
                ...
                if i > stat_window*2+7:
                    ...
                    if self.model_name in TEMPORAL_MODELS:
                        ...
                        self.retrain_model_path="_".join(["cumulative",str(stat_window),str(n_epochs),str(sample),str(threshold),"retrain.model"])
     ...
     #       X_next = scale(X_next)
            ...
            if p_val < threshold and i > stat_window*2+7:
                if verbose == 1:
